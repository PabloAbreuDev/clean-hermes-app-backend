...
describe("Check Health", () => {
  it("Shuold do a request propertly", async () => {
    jest
      ...
      .mockResolvedValue(mockedCheckHealth);
    ...
    expect(mockCheckHealthRepository.findById).toBeCalledWith(
      mockedCheckHealth.id
    );
    expect(mockClient.doRequest).toBeCalledWith(
      mockedCheckHealth.urlToCheck,
      "GET"
    );
  });

  it("Should throw an error if check health report is not found", async () => {
  ...
  it("Should throw an error if check health report is not created", async () => {
    jest
      ...
      .mockResolvedValue(mockedCheckHealth);
    jest
      .spyOn(mockCheckHealthRepository, "addReport")
    ...
    expect(mockCheckHealthRepository.addReport).toBeCalledTimes(2);
  });
});

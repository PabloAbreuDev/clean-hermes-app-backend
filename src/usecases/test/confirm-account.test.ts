...
describe("Confirm account",  ()=>{
    it("Should confirm an account propertly", async()=>{
        jest.spyOn(mockUserRepository, "findByVerifyCode").mockResolvedValue(mockedUser)
        ...
        expect(mockUserRepository.findByVerifyCode).toBeCalledWith(mockedUser.verifyCode)
    ...
    it('Should thrown an error if something went wrong', async ()=>{
        jest.spyOn(mockUserRepository, "findByVerifyCode").mockResolvedValue(mockedUser)
        jest.spyOn(mockUserRepository, "confirmAccount").mockRejectedValue(new Error())
        const confirmAccount = new ConfirmAccount(mockUserRepository)
        await expect(confirmAccount.execute({
            verifyCode: mockedUser.verifyCode 
